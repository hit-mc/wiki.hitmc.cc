...
4. 在管理小群或者私聊中向 Cherrling 索要邀请链接
   ...
   3. ~~出现的原因笔者猜是需要第二套设备作为启动资金~~这是 Tailscale 的预期行为，需要至少另一台设备加入网络后才能接受邀请，可以向管理员反馈协助处理，所以如果自己有自己的其他机器的话，可以加进去，以便顺利接收邀请
5. 在 ssh 目录下生成一对密钥对，参考代码：
      ...
      注意：`-t` 后面指定的是密钥算法类型（如 `ed25519`、`rsa`、`ecdsa` 等），**不是文件名**。如果你想自定义文件名，请使用 `-f` 选项，例如 `-f mykey` 会生成 `mykey`（私钥）和 `mykey.pub`（公钥）。如果不加 `-f`，则会使用默认名称（如 `id_ed25519`）。

      ssh 目录在 Linux / macOS 下是 `~/.ssh`，在 Windows 下推荐使用 `%USERPROFILE%\.ssh` 或直接在 PowerShell / Git Bash 中同样使用 `~/.ssh`（都会解析到用户目录）。命令行生成时公钥和私钥都会默认保存到这个目录里，所以生成完成后去该目录下查找即可。

      中间询问 `passphrase` 的那一步，是**设置一个密码来加密你的私钥文件**（而不是“二次验证”），这样即使私钥被他人获取，也无法直接使用。你可以设置一个强密码，也可以直接回车留空（留空则私钥不加密存储）。其他步骤一路回车就行，如果已经存在同名密钥文件，系统会提示是否覆盖。
6. 将公钥发给 Cherrling 或者 zsh ，用于服务端配置，同时发送你想在机器上留下的 User_ID ，用于后续登录。
7. 配置完成后，可以使用下面的命令登录
...
最后需要注意的是，manx 在 ssh 连接完成之后会显示以下内容
...
2. 安装完成后，需要在启动脚本或特定文件中配置认证登录，同时将 `server.properties` 中 `online-mode` 一项的值设为 `true`
   1. 第一种情况是启动脚本 `run.sh` 包含所有内容，可以直接在参数 `-jar` 前添加以下参数
        ...
        ```bash
        -javaagent:auth.jar=https://hitmc.cc/api/yggdrasil
        ```
...
6. 有些服务端使用 `./start.sh` 或 `bash run.sh` 启动，根据实际情况使用对应的启动脚本。
...
去 `crash-reports` 里面找日志，找最近的，然后可以 cat 打开后全选复制问 ai，或者文件下载到本地然后把文件丢给 ai
